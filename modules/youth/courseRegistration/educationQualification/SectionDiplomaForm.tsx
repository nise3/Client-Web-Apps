...
const SectionDiplomaForm: FC<SectionDiplomaFormProps> = ({
  ...
  useEffect(() => {
    if (getValues) {
      const diplomaInfo: any = getValues('diploma_info');
      const isForeignInstituteValue: any = diplomaInfo?.is_foreign_institute;

      setIsForeignInstitute(isForeignInstituteValue);
      if (diplomaInfo.result) {
        onResultChange(diplomaInfo.result);
      }
    }
  }, [getValues, result]);
