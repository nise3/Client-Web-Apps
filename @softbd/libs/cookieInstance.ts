...
export const setBrowserCookie = (name: string, value: Cookie, options?: CookieSetOptions) => {
  const defaultOptions: CookieSetOptions = {
    path: '/',
    domain: cookieDomain()
  }

  return cookieInstance.set(name, value, typeof options !== "undefined" ? options : defaultOptions)
}

export const removeBrowserCookie = (name: string, options?: CookieSetOptions) => {
  const defaultOptions: CookieSetOptions = {
    path: '/',
    domain: cookieDomain()
  }

  return cookieInstance.remove(name, typeof options !== "undefined" ? options : defaultOptions)
}
