...
export const getMomentDateFormat = (dateValue: any, format = 'MM-DD-YYYY'): string => {
...
export const checkValidImageFormatAndSize = async (file: any) => {
    ...
    try {
        ...
        return (image.height == 750 && image.width == 750) || 'Image should be 750px X 750px';
    } catch (e) {
    }
};
...
export const generatePageNumber = (
    ...
    let startFrom = pageIndex === 1 ? pageIndex : pageIndex === 2 ? pageIndex - 1 : pageIndex - 2;
    return totalPage === 0 ? [] : range(Math.min(totalSlide + startFrom, totalPage), startFrom);
};

export const countPaginatePage = (totalData: number, pageSize: number): number => {
